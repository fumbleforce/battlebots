...
func valid() -> bool:
	return mode in ["windowed", "borderless", "fullscreen"] and resolution.x >= 640 and resolution.y >= 480 and resolution.x <= 7680 and resolution.y <= 4320
...
func save_file(path: String) -> Error:
	...
	config.set_value("video", "version", 1)
...
static func load_file(path: String = DEFAULT_PATH) -> VideoPreferences:
	...
	if not version is int or version != 1 or not saved_mode is String or not saved_size is Vector2i or not saved_vsync is bool:
