...
const REVISION_FIFTEEN_HASHES := ["395b22e42b6263e2f7441c6889f798595533fc376b6a5a241671ddcc16799b5d"]
...
func migrate(data: Dictionary) -> Dictionary:
	...
	if copy.get("schema_version") == 1 and copy.get("loadouts") is Array:
		for index: int in range(copy.loadouts.size()):
			...
			if not draft is Dictionary or draft.get("content_hash") not in REVISION_ONE_HASHES + REVISION_TWO_HASHES + REVISION_THREE_HASHES + REVISION_FOUR_HASHES + REVISION_FIVE_HASHES + REVISION_SIX_HASHES + REVISION_SEVEN_HASHES + REVISION_EIGHT_HASHES + REVISION_NINE_HASHES + REVISION_TEN_HASHES + REVISION_ELEVEN_HASHES + REVISION_TWELVE_HASHES + REVISION_THIRTEEN_HASHES + REVISION_FOURTEEN_HASHES + REVISION_FIFTEEN_HASHES + REVISION_SIXTEEN_HASHES:
				continue
