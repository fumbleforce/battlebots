...
const KINDS := ["vertical_spinner", "horizontal_spinner", "saw", "hammer", "lifter", "ram", "crush", "ram_punch", "spear", "grinder"]
...
func _spawn_burst(origin: Vector3, frame: Basis, kind: String, damage: float) -> void:
	...
	if kind in ["saw", "horizontal_spinner", "vertical_spinner", "hammer", "crush", "ram_punch", "spear", "grinder"]: weight = minf(1.0, weight + 0.25)
	_serial += 1
