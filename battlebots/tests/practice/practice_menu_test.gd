...
func run() -> void:
	...
	await frames()
	check(not game.preview.pause_menu.visible and game.preview.controls_enabled and not game.gameplay_input_allowed(), "Player knockout stays in the arena without opening pause")
	check(game.combat_hud.warning_label.text.contains("RESPAWNING IN 3"), "Knockout shows the respawn countdown: %s" % game.combat_hud.warning_label.text)
	var respawn_frames := ceili(PracticeBotDirector.PLAYER_RESPAWN_SECONDS * Engine.physics_ticks_per_second)
	await frames(respawn_frames - 20)
	check(bot.combat.eliminated, "Player waits out the respawn delay")
	await frames(30)
	check(not bot.combat.eliminated and game.session.local_source() == bot, "Player respawns in place of the same bot after the delay")
	check(game.preview.controls_enabled and not game.preview.pause_menu.visible and not game.combat_hud.warning_label.text.contains("RESPAWNING"), "Respawn returns directly to driving")
	bot.combat.eliminate("test player knockout")
	await frames()
	game._restart_practice.pressed.emit()
	await frames()
	check(not bot.combat.eliminated and game.preview.controls_enabled and not game.preview.resume_button.disabled, "Restart still revives a knocked-out player immediately")
