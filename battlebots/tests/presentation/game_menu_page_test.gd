...
func run() -> void:
	...
	await frames()
	check(game.preview.settings_panel.visible and not game.preview.pause_menu.visible, "Settings replaces game menu")
	...
	await frames()
	check(game.preview.get_node("DiagnosticsLayer").visible and not game.preview.controls_enabled, "Diagnostics stays visible after button-down while controls release")
	...
	game.preview.network_diagnostics.expanded = false
	game.preview.release_controls()
