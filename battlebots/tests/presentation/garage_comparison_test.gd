...
func run() -> void:
	...
	for malformed: Variant in [null, [], {"chassis": "compact"}, {"chassis": 12, "drive": "agile", "weapon": "saw", "armor": "light", "utility": "cooling_pack"}, {"chassis": "agile", "drive": "agile", "weapon": "saw", "armor": "light", "utility": "cooling_pack"}]:
		var broken := original.duplicate(true)
		broken.parts = malformed
		var before := broken.duplicate(true)
		var result := GarageComparison.compare(registry, broken, "armor", "heavy")
	...
	check(not rejected.proposed.valid and rejected.proposed.stats == {"mass": 95.0, "power": 65.0}, "Client-supplied stats never influence comparison")
