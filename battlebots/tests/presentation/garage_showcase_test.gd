...
func run() -> void:
	...
	check(not preview.status.visible and not preview.tooltip_text.is_empty(), "Compact valid showcase moves inspection guidance into its existing tooltip")
	check(preview.rotation_button.text.is_empty() and preview.rotation_button.icon != null, "Compact rotation toggle uses a real icon")
	check(preview.rotation_button.tooltip_text.begins_with("Resume rotation") and preview.rotation_button.accessibility_name == "Resume rotation", "Paused compact toggle exposes the Resume action accessibly")
	for factor: float in [1.0, 1.5, 1.5, 1.0]:
		preview.apply_text_scale(factor)
		await get_tree().process_frame
		check(preview.get_global_rect().encloses(preview.rotation_button.get_global_rect()), "Compact rotation action fits its top-right corner at every text factor")
	preview.rotation_button.pressed.emit()
	check(preview.rotation_button.tooltip_text.begins_with("Pause rotation") and preview.rotation_button.accessibility_name == "Pause rotation", "Resumed compact toggle exposes the Pause action accessibly")
	preview.rotation_button.pressed.emit()
	preview.show_loadout({})
	check(preview.status.visible and preview.status.text.contains("Invalid"), "Compact showcase retains actual validation failure text")
	preview.rotation_button.pressed.emit()
	preview._process(1.0)
