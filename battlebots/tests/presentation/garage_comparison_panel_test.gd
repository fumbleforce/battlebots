...
func run() -> void:
	...
	await choose("armor", "light")
	await settle()
	check(cell(panel.budgets, 3) == "106" and cell(panel.details, 1) == "260", "Repair choice restores valid current stats")
	...
	check(cell(panel.budgets, 3) == "106", "Paint preserves stats")
