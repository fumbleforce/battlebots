...
func run() -> void:
	...
	var primary_before: Array = profile.loadouts[0].cosmetics.sawblade.paint_primary.duplicate()
	...
	check(profile.loadouts[0].cosmetics.sawblade.paint_primary != primary_before, "Selecting paint applies immediately")
