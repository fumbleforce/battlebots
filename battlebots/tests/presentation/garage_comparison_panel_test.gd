...
func run() -> void:
	...
	check(cell(panel.budgets, 5) == "101" and cell(panel.budgets, 6) == "91" and cell(panel.budgets, 7) == "-10", "Mass compares equipped/current/proposed delta")
	check(cell(panel.details, 5) == "8" and cell(panel.details, 6) == "12" and cell(panel.details, 7) == "+4", "Speed uses candidate drive")
	screen.get_node("%Action").pressed.emit()
	await settle()
	check(profile.loadouts[0].parts.drive == "agile", "Equip applies selected part")
	check(cell(panel.budgets, 5) == "91" and cell(panel.budgets, 6) == "91" and cell(panel.budgets, 7) == "0", "Equip refreshes current and proposed")
	screen.undo_button.pressed.emit()
	await settle()
	check(profile.loadouts[0] == original and cell(panel.budgets, 5) == "101" and cell(panel.budgets, 6) == "91", "Undo restores equipped comparison while retaining candidate")
	...
	await settle()
	check(cell(panel.budgets, 5) == "114" and cell(panel.budgets, 6) == "121" and cell(panel.budgets, 7) == "+7", "Overbudget proposal retains verified totals")
	check(cell(panel.details, 1) == "260" and cell(panel.details, 2) == "—" and cell(panel.details, 3) == "—", "Invalid proposal has no invented derived stats or delta")
	check(screen.get_node("%SelDesc").text.contains("Mass exceeds 120 kg"), "Specific candidate error is visible")
	screen.get_node("%Action").pressed.emit()
	await settle()
	check(screen.get_node("%Save").disabled and cell(panel.budgets, 5) == "121", "Invalid equipped draft retains budget and disables Save")
	await choose("armor", "light")
	await settle()
	check(cell(panel.budgets, 6) == "106" and cell(panel.details, 1) == "—" and cell(panel.details, 2) == "260", "Repair candidate restores derived stats only for valid proposed build")
	screen.get_node("%Action").pressed.emit()
	await settle()
	...
	check(cell(panel.budgets, 5) == "106" and cell(panel.budgets, 6) == "106" and cell(panel.budgets, 7) == "0", "Paint preserves stats")
