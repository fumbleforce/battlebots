...
func run() -> void:
	...
	for name: String in ["garage","customize"]:
		...
		if name == "customize":
			...
			check(not screen.get_node("%SelDesc").visible and not screen.get_node("%SelName").visible and screen.details_toggle.visible and screen.details_toggle.icon == screen.DETAILS_OPEN_ICON, "Description starts folded behind a reopen arrow")
			screen.details_toggle.pressed.emit()
			check(screen.get_node("%SelDesc").visible and screen.get_node("%SelName").visible and screen.details_toggle.icon == screen.DETAILS_CLOSE_ICON, "Reopen arrow shows the description with a collapse arrow")
			screen._refresh()
			check(screen.get_node("%SelDesc").visible, "Description stays open across refreshes")
			screen.details_toggle.pressed.emit()
			check(not screen.get_node("%SelDesc").visible, "Collapse arrow folds the description again")
