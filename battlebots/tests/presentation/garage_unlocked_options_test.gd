...
func _ready() -> void:
	...
	profile.loadouts[0].parts.armor = "light"
	...
	for old_hash: String in LoadoutStore.REVISION_FIVE_HASHES:
		...
		check(migrated.loadouts[0].parts == legacy.parts and migrated.loadouts[0].cosmetics == legacy.cosmetics, "Migration preserves legacy compact, selections and appearance")
