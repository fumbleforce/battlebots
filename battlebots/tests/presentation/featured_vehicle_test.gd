...
func run() -> void:
	...
	choices[0].name = str(choices[0].name).left(48)
	featured.render(choices, 0)
	for width: float in [320.0, 560.0, 920.0]:
		featured.size.x = width
		for factor: float in [1.0, 1.25, 1.5, 1.5, 1.0]:
			featured.apply_text_scale(factor)
			await settle()
			var name_rect := featured.name_label.get_global_rect()
			var count_rect := featured.count_label.get_global_rect()
			var previous_rect := featured.previous_button.get_global_rect()
			var next_rect := featured.next_button.get_global_rect()
			check(featured.name_label.get_theme_font_size("font_size") == roundi(24 * factor), "Compact name scaling never compounds")
			check(name_rect.end.x <= count_rect.position.x and count_rect.end.x <= previous_rect.position.x and previous_rect.end.x <= next_rect.position.x, "Compact footer preserves clear separate name/count/actions at each width")
			check(absf(name_rect.get_center().y - next_rect.get_center().y) <= 1, "Compact footer centers name beside the navigation actions")
			check(next_rect.end.x <= featured.get_global_rect().end.x + 1, "Compact footer actions fit available width")
			check(name_rect.position.y >= featured.preview.get_global_rect().end.y, "Compact vehicle identity sits below the expanded preview")
	check(featured.name_label.text_overrun_behavior == TextServer.OVERRUN_TRIM_ELLIPSIS and featured.name_label.tooltip_text == choices[0].name, "Compact long names retain their complete tooltip")
	check(featured.previous_button.tooltip_text == "Previous vehicle" and featured.next_button.accessibility_name == "Next vehicle", "Compact arrows retain clear tooltips and accessible names")
	check(not featured.preview.status.visible, "Compact valid preview hides redundant instructions")
	featured.set_compact(false)
	featured.apply_text_scale(1.5)
	await settle()
	check(featured.name_label.get_theme_font_size("font_size") == 42 and featured.previous_button.text == "PREVIOUS", "Restoring default layout restores original text scaling and labels")
	check(featured.preview.status.visible, "Default preview retains normal instructions")
	featured.queue_free()
	await settle()
	var configured := FeaturedVehicle.new()
	configured.set_compact(true)
	configured.apply_text_scale(1.5)
	configured.render(original, 0)
	configured.size = Vector2(560, 500)
	add_child(configured)
	await settle()
	check(configured.name_label.get_parent() == configured.next_button.get_parent() and configured.name_label.get_theme_font_size("font_size") == 36, "Compact mode and text scale can be configured before ready")
	configured.free()
