...
func run() -> void:
	...
	check(game.hud_settings.text_scale_choice.get_theme_font_size("font_size") == 30, "Accessibility controls enlarge themselves")
	...
	await frames()
	check(game._menu_text_scale == 1.0 and game.hud_settings.text_scale_choice.get_theme_font_size("font_size") == 20, "Cancel restores menu and settings text")
