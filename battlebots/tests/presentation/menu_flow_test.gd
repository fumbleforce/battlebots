...
func join_flow() -> void:
	...
	check(not lobby.get_node("Layout/Body/Row/Match/ArenaCard").is_visible_in_tree(), "Offline Join does not require or imply a map selection")
...
func host_flow() -> void:
	...
	lobby.build_choice.select(0)
	lobby.build_choice.item_selected.emit(0)
	...
	lobby.build_choice.select(1)
	lobby.build_choice.item_selected.emit(1)
	lobby.build_button.pressed.emit()
	await frames()
	await press("Back")
