...
func run() -> void:
	...
	var five_index: int = host.player_count_choice.get_item_index(10)
	check(five_index >= 0, "Host menu offers five versus five")
	host.player_count_choice.select(five_index)
	host.player_count_choice.item_selected.emit(five_index)
	host.host_game()
	await frames(3)
	check(host.session.player_capacity == 10 and host.session.lobby_view.mode == "5v5", "Selected five versus five opens a ten-slot lobby")
	check(host.session.match_state.snapshot().capacity == 10 and host.session.match_view.mode == "5v5",
		"Match metadata already agrees with lobby before players ready")
	check(host.menu_title.text == "Lobby / 5v5" and "1/10 players joined" in host.menu_hint.text,
		"Menu explains the selected mode and missing players")
	host.session.leave()
	host.player_count_choice.select(0)
	host.player_count_choice.item_selected.emit(0)
	host.host_game()
	await frames(3)
