...
Public 2v2 requires four connected players; matches do not start with empty slots. 5v5 uses full custom lobbies at launch. FFA supports custom lobbies and starts with at least four players. Practice supports one player with stationary or simple driving targets. Practice Duel is the same offline mode with a single stationary, non-aggressive Atlas MX at the arena centre and no other NPCs, Woodland giant, cooling zones or coolant canisters, plus a tight block of six stationary monowheels on the player's left. Its Esc menu is a session-only tuning panel (#84): the player's weapons (Weapon 1 and the Weapon 2 utility slot: part pickers, allow auto fire, fire rate, damage, range, knockback, recoil, weight, piercing, area of effect, stagger), body (chassis picker, heat toggle, health, weight, armour pieces) and movement (jump cooldown toggle, max speed, acceleration, grip, turn speed, jump force, nitro boost). Nothing there is saved or reaches LAN or online play. AI does not fill PvP vacancies.
