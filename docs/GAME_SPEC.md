...
Public 2v2 requires four connected players; matches do not start with empty slots. 5v5 uses full custom lobbies at launch. FFA supports custom lobbies and starts with at least four players. Practice supports one player with stationary or simple driving targets. Practice Duel is the same offline mode without the training NPCs, roamers, Woodland giant, cooling zones, coolant canisters or the centre pickup. The player starts in the middle of the room, facing as from its usual lane (#90). Seen from there it has a tight block of six stationary monowheels on the left, a stationary, non-aggressive Atlas MX straight ahead facing the player, and a second non-aggressive Atlas MX on the right that drives forward and back along the player's forward axis; both Atlases stand as far from their walls as the monowheel block does from the left wall (#88). Its Esc menu is a session-only tuning panel (#84): the player's weapons (Weapon 1 and the Weapon 2 utility slot: part pickers, allow auto fire, fire rate, damage, range, knockback, recoil, weight, piercing, area of effect, stagger strength in %, camera recoil), body (chassis picker, heat toggle, health, weight, armour pieces) and movement (jump cooldown toggle, max speed, acceleration, grip, turn speed, jump force, nitro boost). Nothing there is saved or reaches LAN or online play. AI does not fill PvP vacancies.
...
Builds have no mass limit (#49, user request 23 September 2026); mass is paid for in acceleration and top speed instead. All builds share a **100-unit installed power** budget. Power budget controls legal construction; heat is the sole runtime operating resource. Chassis fixes socket locations, collision envelope, core integrity, and recovery mechanism. No part can extend beyond its permitted weapon sweep or deployment envelope.

**User revision, 20 September 2026:** machines are three times their original size on every axis. Canonical hulls are now 4.5–6.6 meters long, 3.6–5.4 meters wide and 1.5 meters tall; the active balanced hull is 4.8 × 1.5 × 6.0 meters. Weapons, drive assemblies, collision and effects follow the physical size. The subsequent 23 September revision expands Foundry to 100 meters across. These are deliberately oversized game machines; mass remains a gameplay stat (there is no longer a mass ceiling, #49). See [implementation and checks](coordination/B_HEAVY_MACHINES.md).
...
Unused mass is permitted. Final mass influences acceleration, resistance to impulses, and recovery. Top speed starts from the drive package and scales with mass (#49): × clamp(1 + (100 − mass kg) × 0.005, 0.6, 1.1), so a 100 kg build keeps its drive speed, lighter builds gain up to 10% and heavier builds lose speed down to 60% of it at 180 kg (tuning in `data/bot_physics.json` `mass`). Grip caps drive force so mass is not an unlimited pushing advantage.
