...
try {
    foreach ($index in 0..$PlayerCount) {
        $mode = if ($index -eq 0) { "--server --players=$PlayerCount --port=$port" } else { "--join=127.0.0.1 --port=$port --ready" }
        $arguments = "--headless --path `"$projectRoot`" --max-fps 60 --quit-after 2400 -- $mode"
